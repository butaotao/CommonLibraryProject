...
public class AVVideoControl {
	...
	private EnableCameraCompleteCallback mEnableCameraCompleteCallback = new EnableCameraCompleteCallback() {
		protected void onComplete(boolean enable, int result) {
			...
			mContext.sendBroadcast(new Intent(VChatUtil.ACTION_ENABLE_CAMERA_COMPLETE).putExtra(VChatUtil.EXTRA_AV_ERROR_RESULT, result).putExtra(VChatUtil.EXTRA_IS_ENABLE, enable));
		}
	};
	...
	private EnableExternalCaptureCompleteCallback mEnableExternalCaptureCompleteCallback = new EnableExternalCaptureCompleteCallback() {
			...
			protected void onComplete(boolean enable, int result) {
				...
				mContext.sendBroadcast(new Intent(VChatUtil.ACTION_ENABLE_EXTERNAL_CAPTURE_COMPLETE).putExtra(VChatUtil.EXTRA_AV_ERROR_RESULT, result).putExtra(VChatUtil.EXTRA_IS_ENABLE, enable));
	...
	private SwitchCameraCompleteCallback mSwitchCameraCompleteCallback = new SwitchCameraCompleteCallback() {
		protected void onComplete(int cameraId, int result) {
			...
			mContext.sendBroadcast(new Intent(VChatUtil.ACTION_SWITCH_CAMERA_COMPLETE).putExtra(VChatUtil.EXTRA_AV_ERROR_RESULT, result).putExtra(VChatUtil.EXTRA_IS_FRONT, isFront));
		}
	};
	...
	RemoteVideoPreviewCallback remoteVideoPreviewCallback = new RemoteVideoPreviewCallback() {
		public void onFrameReceive(VideoFrame videoFrame) {
			...
			String printTxtPath =   VChatUtil.outputYuvFilePath + "/" + videoFrame.height + "_" + videoFrame.width + "_" + videoFrame.identifier +".yuv" ;
