...
//		mAVEndpointControl.initMembersUI((MultiVideoMembersControlUI) contentView.findViewById(R.id.qav_gaudio_gridlayout));
	...
	public void onResume() {
		if(mAVContextControl.getAVContext()!=null)
...
//		}
		...
		if (getAVContext() != null&&getAVContext().getAudioCtrl() != null) {
			getAVContext().getAudioCtrl().stopTRAEService();
		}
		exitRoom();
	}
...
//			mAVUIControl.setRemoteHasVideo(identifier,videoSrcType,isRemoteHasVideo,false);
		}
	}
	...
	public void requestAllMember(){
		if(mAVEndpointControl!=null)
			mAVEndpointControl.requestMembers(getMemberList());
	}

	public void enableSpeaker(boolean enable){
        if(!hasAVContext())return;
        getAVContext().getAudioCtrl().enableSpeaker(enable);
        mIsSpeakerEnable=enable;
	}
