...
public class ExternalCaptureThread extends Thread {
	...
	public void run() {
		...
		int frameBytesNumber = VChatUtil.yuvHigh * VChatUtil.yuvWide * 3 / 2;
		while (canRun) {
			
			byte[] bytesInOneFrame = new byte[frameBytesNumber];
			
			//可以设置成手机里面保存的yuv文件
			String fileName = VChatUtil.inputYuvFilePath;
			...
			try {
				...
				while ((byteRead = in.read(bytesInOneFrame)) != -1) {
					...
					avVideoCtrl.fillExternalCaptureFrame(bytesInOneFrame, frameBytesNumber,
							VChatUtil.yuvWide, VChatUtil.yuvHigh, 0, VChatUtil.yuvFormat, 1);
					sleep(40);
				}
