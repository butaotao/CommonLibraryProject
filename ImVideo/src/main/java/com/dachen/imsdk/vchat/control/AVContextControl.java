...
class AVContextControl {
	...
	private static int DEMO_SDK_APP_ID = VChatUtil.DEFAULT_SDK_APP_ID;
	private static String DEMO_ACCOUNT_TYPE = VChatUtil.DEFAULT_ACCOUNT_TYPE;
	...
	private AVContext.StartContextCompleteCallback mStartContextCompleteCallback = new AVContext.StartContextCompleteCallback() {
		public void OnComplete(int result) {
			...
			mContext.sendBroadcast(new Intent(
					VChatUtil.ACTION_START_CONTEXT_COMPLETE).putExtra(
					VChatUtil.EXTRA_AV_ERROR_RESULT, result));
...
//			if (!TextUtils.isEmpty(VChatUtil.modifyAppid)) {
//			  DEMO_SDK_APP_ID = Integer.parseInt(VChatUtil.modifyAppid);
//			}
//			if (!TextUtils.isEmpty(VChatUtil.modifyUid)) {
//			  DEMO_ACCOUNT_TYPE = VChatUtil.modifyUid;
//			}
//			VChatUtil.modifyAppid = String.valueOf(DEMO_SDK_APP_ID);
//			VChatUtil.modifyUid = DEMO_ACCOUNT_TYPE;
			DEMO_SDK_APP_ID =VChatUtil.DEFAULT_SDK_APP_ID;
			DEMO_ACCOUNT_TYPE = VChatUtil.DEFAULT_ACCOUNT_TYPE;
...
//		TIMManager.getInstance().setEnv(1);
	...
	private void onLogout(boolean result)
	{
		...
		mContext.sendBroadcast(new Intent(
				VChatUtil.ACTION_CLOSE_CONTEXT_COMPLETE));
	}

	boolean isDefaultAppid() {
		if (DEMO_SDK_APP_ID == VChatUtil.DEFAULT_SDK_APP_ID)
		  return true;
		else
		  return false;
	}
	...
	boolean isDefaultUid() {
		return DEMO_ACCOUNT_TYPE.equals(VChatUtil.DEFAULT_ACCOUNT_TYPE);
	}
}