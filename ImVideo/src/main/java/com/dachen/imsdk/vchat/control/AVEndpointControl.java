...
import com.dachen.imsdk.vchat.MultiVideoMembersControlUI;
import com.dachen.imsdk.vchat.MultiVideoMembersControlUI.MultiVideoMembersClickListener;
			...
			else
			{
				...
				if(hasRequest)
				{
					mContext.sendBroadcast(new Intent(
							VChatUtil.ACTION_VIDEO_CLOSE).putExtra(
							VChatUtil.EXTRA_IDENTIFIER, mRequestIdentifierList[index]).putExtra(
							VChatUtil.EXTRA_VIDEO_SRC_TYPE, mRequestViewList[index].videoSrcType));
				...
				else
				{
					...
					*/
					...
					AVEndpoint.requestViewList(mRequestIdentifierList, mRequestViewList, mRequestCount, mRequestViewListCompleteCallback);
					mContext.sendBroadcast(new Intent(
							VChatUtil.ACTION_VIDEO_SHOW).putExtra(
							VChatUtil.EXTRA_IDENTIFIER, identifier).putExtra(
							VChatUtil.EXTRA_VIDEO_SRC_TYPE, view.videoSrcType));
				}
			}						
...
//		mRequestViewList = new AVView[AVView.MAX_VIEW_COUNT];
		mRequestIdentifierList = new String[VChatUtil.VIDEO_MAX_COUNT];
		mRequestViewList = new AVView[VChatUtil.VIDEO_MAX_COUNT];
		mRequestCount = 0;
	}
	...
	public void closeRemoteVideo() {
		...
		mContext.sendBroadcast(new Intent(VChatUtil.ACTION_VIDEO_CLOSE));
	}
	...
	private boolean deleteRequestView(int index){
		...
		mRequestIdentifierList = requestIdentifierArrayList.toArray(new String[VChatUtil.VIDEO_MAX_COUNT]);
		mRequestViewList = requestViewArrayList.toArray(new AVView[VChatUtil.VIDEO_MAX_COUNT]);
		mRequestCount--;
...
//		mRequestViewList = new AVView[AVView.MAX_VIEW_COUNT];
	...
	public void requestMembers(ArrayList<MemberInfo> list){
		for (MemberInfo info :list) {
			addOneMemberRequest(info);
		}
		AVEndpoint.requestViewList(mRequestIdentifierList, mRequestViewList, mRequestCount, mRequestViewListCompleteCallback);
	}
	public void addOneMemberRequest(MemberInfo info){
		int videoSrcType;
...
//			}
			...
			*/
			...
			UserInfo u= VChatManager.getInstance().removeInvite(identifier,false);
