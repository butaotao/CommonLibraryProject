...
//	private AVRoomMulti.Delegate mRoomDelegate = new AVRoomMulti.Delegate() {
	private class RoomDelegate implements AVRoomMulti.Delegate{
		int roomId;

		public RoomDelegate(int roomId) {
			this.roomId = roomId;
		}
		...
		public void onEnterRoomComplete(int result) {
			...
			VChatManager.getInstance().curRoomId=roomId;
			mContext.sendBroadcast(new Intent(VChatUtil.ACTION_ROOM_CREATE_COMPLETE).putExtra(VChatUtil.EXTRA_AV_ERROR_RESULT, result));
		}
		...
		public void onExitRoomComplete(int result) {
            ...
            ImVideo.getInstance().mQavsdkControl.initAVVideo();
			ImVideo.getInstance().mQavsdkControl.stopContext();
			mContext.sendBroadcast(new Intent(VChatUtil.ACTION_CLOSE_ROOM_COMPLETE));
		}
/*
...
*/
		...
		public void onChangeAuthority(int retCode) {
			...
			mContext.sendBroadcast(new Intent(VChatUtil.ACTION_CHANGE_AUTHRITY).putExtra(VChatUtil.EXTRA_AV_ERROR_RESULT, retCode));
		}
	};
			...
			if(endpoint == null)//endpoint is not exist at all
			{
				...
				for (int j = 0; j < mAudioAndCameraMemberList.size(); j++)
				{
					if (mAudioAndCameraMemberList.get(j).identifier.equals(updateList[i]))
					{
						...
						VChatManager.getInstance().removeUser(updateList[i],true);
                        removed=true;
						break;
					}
				}
...
//                if(!removed)
//                    VChatManager.getInstance().removeUser(updateList[i]);
                continue;
			}
		...
		mContext.sendBroadcast(new Intent( VChatUtil.ACTION_MEMBER_CHANGE));
	}
...
//		AVRoom.EnterRoomParam enterRoomParam = new AVRoomMulti.EnterRoomParam(relationId, VChatUtil.auth_bits, authBuffer, roomRole, audioCat,true);
		AVRoom.EnterRoomParam enterRoomParam = new AVRoomMulti.EnterRoomParam(relationId, VChatUtil.auth_bits, authBuffer, roomRole, audioCat, true);
		// create room
//		avContext.enterRoom(AVRoom.AV_ROOM_MULTI, mRoomDelegate, enterRoomParam);
		avContext.enterRoom(AVRoom.AV_ROOM_MULTI, new RoomDelegate(relationId), enterRoomParam);
