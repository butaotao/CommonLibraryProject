...
public class LoopViewPager extends ViewPager {
	...
	private void addDot() {
		if (dotParent != null) {
			// int len = 0;
			// if(mAdapter!=null){
			// len = mAdapter.getCount()-2;
			// }
			dotParent.removeAllViews();
			int len = mAdapter.getCount() - 2;
			dots = new ArrayList<CheckBox>();
			for (int i = 0; i < len; i++) {
				View view = LayoutInflater.from(ctx).inflate(
						R.layout.loop_view_pager_dot, null);
				CheckBox dot = (CheckBox) view
						.findViewById(R.id.loop_view_pager_dot_ck);
	...
	private void startImageTimerTask() {
		...
		if (mAdapter.getCount() == 1) {
			...
			if (mAdapter != null && mAdapter.getCount() - 2 > 1 && isStop) {
