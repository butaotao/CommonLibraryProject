...
public class ChatFaceView extends RelativeLayout {
	...
	private void init(Context context) {
		...
		LayoutInflater.from(mContext).inflate(R.layout.face_view, this);
	...
	public void switchViewPager3() {
		List<Map<String, Integer>> expressionData = ExpressionParser.New_Smiles.getExpressionData();
		if (expressionData == null) {
			return;
		}
		int pageCount = expressionData.size();
		mPoints = new ImageView[pageCount];// 有几页表情
		mPointsLayout.removeAllViews();
		for (int i = 0; i < mPoints.length; i++) {
			ImageView point = new ImageView(mContext);
			if (i == 0) {
				point.setBackgroundResource(R.drawable.tab_press);
			} else {
				point.setBackgroundResource(R.drawable.tab_normal);
			}
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(dip_To_px(mContext, 5),
					dip_To_px(mContext, 5));
		...
		for (int i = 0; i < pageCount; i++) {
			...
			EmotionImgAdapter adapter = new EmotionImgAdapter(mContext, getExpressionResId(expressionData.get(i)), 1);
			gridView.setAdapter(adapter);
			gridView.setOnItemClickListener(new EmotionItemClick(i, getExpressionText(expressionData.get(i)), 1));
	...
	private int[] getExpressionResId(Map<String, Integer> map) {
		...
		int[] resId = new int[map.size()];
		int index = 0;
		for (Entry<String, Integer> entry : map.entrySet()) {
			resId[index++] = entry.getValue();
		}
		return resId;
	}
	...
	class EmotionItemClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			String insertText = strArray[position];
	...
	class PagerListener implements OnPageChangeListener {
		...
		public void onPageSelected(int arg0) {
			for (int i = 0; i < imgArray.length; i++) {
				imgArray[i].setBackgroundResource(i == arg0 ? R.drawable.tab_press : R.drawable.tab_normal);
			}
		}
	...
	class EmotionImgAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (switchID == 2) {
				LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(mScreenWidth / 4,
						mScreenWidth / 4);
